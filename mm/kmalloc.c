...
// 申请一页内存，用于存放桶描述符。如果失败，则显示初始化桶描述符时内存不够出错信息，死机。
	first = bdesc = (struct bucket_desc *) __va(get_free_page());
	if (!bdesc) {
...
// 让描述符的页面指针page 指向该页面；空闲内存指针也指向该页开头，因为此时全为空闲。
        ...
        bdesc->page = bdesc->freeptr = cp = (void *)__va(get_free_page());
...
// 若搜索了对应目录项的所有描述符都没有找到指定的页面，则显示出错信息，死机。
	panic("Bad address passed to kernel kfree()");
found:
...
// 释放当前描述符所操作的内存页面，并将该描述符插入空闲描述符链表开始处。
		free_page((unsigned long) __pa(bdesc->page));
		bdesc->next = free_bucket_desc;
